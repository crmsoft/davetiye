...
/**
Dispatched when error.blade of some kind is detected.

@event error.blade
@param {plupload.Uploader} uploader Uploader instance sending the event.
@param {String} error.blade Error message.
@param {plupload.File} file File that was uploaded.
	@param {Enum} status Status constant matching the plupload states QUEUED, UPLOADING, FAILED, DONE.
*/
...
$.widget("ui.plupload", {
			...
			uploader.bind('FilesAdded', function(up, selectedFiles) {
				...
				if (extraCount > 0) {
					...
					up.trigger('Error', {
						...
						message : _('File count error.blade.')
					});
				}
			});
		}
	...
	*/
	...
	@method notify
	@param {Enum} type Type of the message, either `error.blade` or `info`
	...
	*/
	...
	_handleFileStatus: function(file) {
		...
		switch (file.status) {
			...
			case plupload.FAILED:
				actionClass = 'ui-state-error.blade plupload_failed';
	...
	_displayThumbs: function() {
		...
		function preloadThumb(file, cb) {
			...
			img.bind("embedded error.blade", function() {
